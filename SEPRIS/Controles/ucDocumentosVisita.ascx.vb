...
Imports Utilerias
Public Class ucDocumentosVisita
...
#Region "Props"
    ...
    Public Sub ModificaIds(piIdVisita As Integer)
        ...
        If Not ddlAreaFirma.CssClass.Contains("_" & piIdVisita.ToString()) Then ddlAreaFirma.CssClass = ddlAreaFirma.ID & "_" & piIdVisita.ToString()
        If Not lstUsuariosFirma.CssClass.Contains("_" & piIdVisita.ToString()) Then lstUsuariosFirma.CssClass = lstUsuariosFirma.ID & "_" & piIdVisita.ToString()
        If Not lstFirmas.CssClass.Contains("_" & piIdVisita.ToString()) Then lstFirmas.CssClass = lstFirmas.ID & "_" & piIdVisita.ToString()

        If Not ddlAreaRubrica.CssClass.Contains("_" & piIdVisita.ToString()) Then ddlAreaRubrica.CssClass = ddlAreaRubrica.ID & "_" & piIdVisita.ToString()
        If Not lstUsuariosRubrica.CssClass.Contains("_" & piIdVisita.ToString()) Then lstUsuariosRubrica.CssClass = lstUsuariosRubrica.ID & "_" & piIdVisita.ToString()
        If Not lstRubricas.CssClass.Contains("_" & piIdVisita.ToString()) Then lstRubricas.CssClass = lstRubricas.ID & "_" & piIdVisita.ToString()

        'If Not chkFiltroSIE.CssClass.Contains("_" & piIdVisita.ToString()) Then chkFiltroSIE.CssClass = chkFiltroSIE.ID & "_" & piIdVisita.ToString()
        If Not txtAsuntoOficio.CssClass.Contains("_" & piIdVisita.ToString()) Then txtAsuntoOficio.CssClass = txtAsuntoOficio.ID & "_" & piIdVisita.ToString()
        'If Not txtAsuntoOficio.Attributes.Equals("_" & piIdVisita.ToString()) Then txtAsuntoOficio.CssClass = txtAsuntoOficio.ID & "_" & piIdVisita.ToString()
        If Not txtComentariosOficio.CssClass.Contains("_" & piIdVisita.ToString()) Then txtComentariosOficio.CssClass = txtComentariosOficio.ID & "_" & piIdVisita.ToString()
        If Not txtDestinatarioOficio.CssClass.Contains("_" & piIdVisita.ToString()) Then txtDestinatarioOficio.CssClass = txtDestinatarioOficio.ID & "_" & piIdVisita.ToString()
        If Not ddlPuestoDestinatario.CssClass.Contains("_" & piIdVisita.ToString()) Then ddlPuestoDestinatario.CssClass = ddlPuestoDestinatario.ID & "_" & piIdVisita.ToString()
        If Not ddlAreaOficio.CssClass.Contains("_" & piIdVisita.ToString()) Then ddlAreaOficio.CssClass = ddlAreaOficio.ID & "_" & piIdVisita.ToString()
        If Not divRegistroSICOD.Attributes.Equals(divRegistroSICOD.ID & "_" & piIdVisita.ToString()) Then divRegistroSICOD.Attributes.Add("class", divRegistroSICOD.ID & "_" & piIdVisita.ToString())
                ...
                Select Case pObjPropiedades.ppObjVisita.IdEstatusActual
                    Case Constantes.EstatusPaso.EnRevisionEspera, Constantes.EstatusPaso.EnAjustes ''Flujo inicial paso 17, EnRevisionEspera, ''Flujo 2 paso 17, EnAjustes
                        Select Case puObjUsuario.IdentificadorPerfilActual
                            Case Constantes.PERFIL_ADM, Constantes.PERFIL_SUP, Constantes.ABOGADOS.PERFIL_ABO_ASESOR, Constantes.PERFIL_INS, Constantes.PERFIL_SOLO_CARGA 'El Supervisor / Superior  puede:  *Enviar documentos a revisión y por lo tanto dar vo.bo, el abogado e inspector pueden adjuntar documentos (hacer ajustes a los docs)
                                Select Case puObjUsuario.IdArea
                                    Case Constantes.AREA_PLD, Constantes.AREA_VO, Constantes.AREA_VF, Constantes.AREA_CGIV ', Constantes.AREA_VJ 'Usuarios VJ no pueden cargar archivos en paso 17
                                        lbTienePermiso = True
                                    Case Else
                                        lbTienePermiso = False
                                End Select
                    ...
                    Case Constantes.EstatusPaso.Revisado, Constantes.EstatusPaso.AjustesRealizados ''Flujo inicial paso 17, Revisado, ''Flujo inicial paso 17, Revisado, regresa paso 16
                        Select Case puObjUsuario.IdentificadorPerfilActual
                            Case Constantes.PERFIL_ADM, Constantes.PERFIL_SUP, Constantes.PERFIL_SOLO_CARGA ''Flujo 2 paso 17, AjustesRealizados, ''Flujo 2 paso 17 regresa paso 16, AjustesRealizados
                                Select Case puObjUsuario.IdArea
                                    Case Constantes.AREA_PR ', Constantes.AREA_VJ 'Usuarios VJ no pueden cargar archivos en paso 17
                                        lbTienePermiso = True
                                    Case Else
                                        lbTienePermiso = False
                                End Select
...
#End Region
    ...
    Private Sub CargarArchivoSharePoint(sender As Object, e As CommandEventArgs)
        ...
        Dim btnSicod As Button = gvGridRow.FindControl("btnSicod" & lsNomControl)
        ...
        If e.Row.RowType = DataControlRowType.DataRow And pObjPropiedades.peObjExp.HayDocumentos And Not IsNothing(e.Row.DataItem) Then
            ...
            Try
                If Not IsDBNull(DataBinder.Eval(e.Row.DataItem, "N_ID_DOCUMENTO")) Then
                    ...
                    btnBuscarSICOD = New Button
                    btnBuscarSICOD.ID = "btnBuscarSICOD"
                    'btnBuscarSICOD.CssClass = "botones"
                    btnBuscarSICOD.Text = "Buscar"
                    btnBuscarSICOD.Width = 50
                    ...
                    If lstDocumento.Count > 0 Then
                        ...
                        For Each objDocumento As Documento In lstDocumento
                            If Not IsNothing(objDocumento) Then
                                ...
                                ''Si es paso 13 debe estar visible el boton
                                'If pObjPropiedades.ppObjVisita.IdPasoActual = 13 And liNumDoc = 34 Then
                                            ...
                                            If EstadoDoc = Constantes.EstatusPaso.Nulidad Or EstadoDoc = Constantes.EstatusPaso.Rechazado Then
                                                ...
                                                If Not ExtArchivo.Equals(".pdf") Then
                                                    If Not ExtArchivo.Equals(".zip") Then ''AMMM Este If se agrego para que los zip tampoco muestre el botón de enviar
                                                        ...
                                                        If objDocumento.FolioSICOD.Length = 0 Or Nothing Then
                                ...
                                If objDocumento.N_FLAG_SICOD = Constantes.Verdadero Then
                                    e.Row.Cells(piColumnaDocs + 2).Controls.Add(btnBuscarSICOD)
                                End If


                                'If objDocumento.N_FLAG_SICOD = Constantes.Verdadero And objDocumento.I_ID_PASO_INI >= pObjPropiedades.ppObjVisita.IdPasoActual And objDocumento.I_ID_PASO_FIN <= pObjPropiedades.ppObjVisita.IdPasoActual Then
                                '    e.Row.Cells(piColumnaDocs + 2).Controls.Add(btnBuscarSICOD)
                                '    btnBuscarSICOD.Visible = True
                                'End If
                ...
                If Not IsNothing(btnRegistroSICOD) Then
                    ...
                    btnRegistroSICOD.Visible = IIf(lbRegSicod = Constantes.Verdadero, lbPermiteCarga, False)
                    ''Si es paso 13 debe estar visible el boton
                    'If pObjPropiedades.ppObjVisita.IdPasoActual = 13 And liNumDoc = 34 Then
                    '   btnRegistroSICOD.Visible = True
                    'End If
                    btnRegistroSICOD.OnClientClick = "RegistroSICOD(" + e.Row.DataItem("N_ID_DOCUMENTO").ToString() + ", 1, " + pObjPropiedades.ppObjVisita.IdVisitaGenerado.ToString() + ", '" + liClasifSicod.ToString() + "', '" + liTipoOficio.ToString() + "', '" + liPasoIni.ToString + "'); return false;"
                End If
