<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="ucDocumentosVisita.ascx.vb" Inherits="SEPRIS.ucDocumentosVisita" %>
...
<script type="text/javascript">
    ...
    function LevantaVentanaOficioNvo(idRen, idDocumento, idCatDocumento, idVersion, idNumVersiones, idVisita, idArea, tipo, paso) {
        ...
        if (nav.indexOf("msie") != -1) {
            ...
            //var modal =
            window.open(url + "?r=" + idRen + "&ds=" + idDocumento + "&idCat=" + idCatDocumento + "&idArea=" + idArea + "&tipo=" + tipo + "&vds=" + idVersion + "&idFolio=" + idVisita + "&paso=" + paso + "&origen=VISITA", "SICOD", winprops);
    ...
    function RegistroSICOD(IdCatDocumento, IdDocumento, idFolio, clasificacion, idTipoOficio, idPaso) {
        ...
        var divs = $(".divRegistroSICOD_" + idFolio).toArray().length;
        if (divs == 2) {
            $(".divRegistroSICOD_" + idFolio).remove();
        }
        ...
        $(".divRegistroSICOD_" + idFolio + " #chkFiltroSIE").addClass("chkFiltroSIE_" + idFolio);
        $("#txtAsuntoOficio").val("");
        $(".divRegistroSICOD_" + idFolio +" #txtAsuntoOficio").addClass("txtAsuntoOficio_" + idFolio);
        $("#txtComentariosOficio").val("");
        $(".divRegistroSICOD_" + idFolio + " #txtComentariosOficio").addClass("txtComentariosOficio_" + idFolio);
        $("#txtDestinatarioOficio").val("");
        $(".divRegistroSICOD_" + idFolio + " #txtDestinatarioOficio").addClass("txtDestinatarioOficio_" + idFolio);
        ...
        $(".divRegistroSICOD_" + idFolio).dialog({
            resizable: false,
            autoOpen: true,
            height: 500,
            width: 700,
            ...
            buttons: {
                "Aceptar": function () {
                    ...
                    if ($(".txtAsuntoOficio_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() == "") {
                        conterr++;
                        err += "<li>El asunto es obligatorio.</li>";
                    }
                    if ($(".txtComentariosOficio_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() == "") {
                        conterr++;
                        err += "<li>Los comentarios son obligatorios.</li>";
                    }
                    if ($(".txtDestinatarioOficio_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() == "") {
                        conterr++;
                        err += "<li>El destinatario es obligatorio.</li>";
                    }
                    if ($(".ddlPuestoDestinatario_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() == "-1") {
                        conterr++;
                        err += "<li>El puesto es obligatorio.</li>";
                    }
                    if ($(".lstFirmas_"+ $("#<%=hdTIDFolio.ClientID%>").val() +  " option").length == 0) {
                        conterr++;
            ...
            $("#divConfirmacion").dialog({
                ...
                buttons: {
                    "Aceptar": function () {
                        if (Respuesta == 0) {
                            $(this).dialog("close");
                            $(".divRegistroSICOD_" + $("#<%=hdTIDFolio.ClientID%>").val()).dialog("close");
                        }
                        else {
                            CrearFolio();
                            //
    ...
    function CrearFolio() {
        ...
        var Rubricas = '';
        var EsSIE = 0;
        let lstFirmasVisitaActual = ".lstFirmas_" + $("#<%=hdTIDFolio.ClientID%>").val();
        $(lstFirmasVisitaActual + " option").each(function (i, selected) {
            Firmas += $(selected).val() + ',';
        });
        let lstRubricasVisitaActual = ".lstRubricas_" + $("#<%=hdTIDFolio.ClientID%>").val();
        $(lstRubricasVisitaActual + " option").each(function (i, selected) {
            Rubricas += $(selected).val() + ',';
        });

        let chkFiltroSIEVisitaActual = ".chkFiltroSIE_" + $("#<%=hdTIDFolio.ClientID%>").val();
        if ($(chkFiltroSIEVisitaActual).prop('checked')) {
            EsSIE = 1;
        }

        if ($("#<%=hdIdDocumento.ClientID%>").val() > 0)
            param_data = '{Folio:' + $("#<%=hdTIDFolio.ClientID%>").val() + ', IdCatDocumento:' + $("#<%=hdCatDocumento.ClientID%>").val() + ', IdDocumento: ' + $("#<%=hdIdDocumento.ClientID%>").val() + ', Firmas: "' + Firmas + '", Rubricas: "' + Rubricas + '", Destinatario: "' + $(".txtDestinatarioOficio_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() + '", Puesto: ' + $(".ddlPuestoDestinatario_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() + ', EsSIE: ' + EsSIE + ', Asunto: "' + $(".txtAsuntoOficio_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() + '", Comentarios: "' + $(".txtComentariosOficio_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() + '", UsuarioElaboro: "' + '<%=Usuario%>' + '", Area: "' + $(".ddlAreaOficio_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() + '", TipoDocumentoP: "' + $("#<%=hdIdTipoOficio.ClientID%>").val() + '", Clasificacion: "' + $("#<%=hdClasif.ClientID%>").val() + '", Paso: ' + $("#<%=hdIdPaso.ClientID%>").val() + ', Proceso:6 }';
        else
            param_data = '{Folio:' + $("#<%=hdTIDFolio.ClientID%>").val() + ', IdCatDocumento:' + $("#<%=hdCatDocumento.ClientID%>").val() + ', IdDocumento: ' + $("#<%=hdIdDocumento.ClientID%>").val() + ', Firmas: "' + Firmas + '", Rubricas: "' + Rubricas + '", Destinatario: "' + $(".txtDestinatarioOficio_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() + '", Puesto: ' + $(".ddlPuestoDestinatario_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() + ', EsSIE: ' + EsSIE + ', Asunto: "' + $(".txtAsuntoOficio_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() + '", Comentarios: "' + $(".txtComentariosOficio_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() + '", UsuarioElaboro: "' + '<%=Usuario%>' + '", Area: "' + $(".ddlAreaOficio_" + $("#<%=hdTIDFolio.ClientID%>").val()).val() + '", TipoDocumentoP: "' + $("#<%=hdIdTipoOficio.ClientID%>").val() + '", Clasificacion: "' + $("#<%=hdClasif.ClientID%>").val() + '", Paso: ' + $("#<%=hdIdPaso.ClientID%>").val() + ', Proceso:5 }';
    ...
    function AgregarFirma() {
        ...
        //}
        let lsUsuariosFirmaVisitaActual = ".lstUsuariosFirma_" + $("#<%=hdTIDFolio.ClientID%>").val();
        let lstFirmasVisitaActual = ".lstFirmas_" + $("#<%=hdTIDFolio.ClientID%>").val();
        if ($(lsUsuariosFirmaVisitaActual + " option:selected").length > 0) {
            $(lstFirmasVisitaActual).append($("<option></option>").attr("value", $(lsUsuariosFirmaVisitaActual + " option:selected").val()).text($(lsUsuariosFirmaVisitaActual + " option[value='" + $(lsUsuariosFirmaVisitaActual).val() + "']").text()));
            $(lstFirmasVisitaActual).val($(lsUsuariosFirmaVisitaActual).val());
            $(lsUsuariosFirmaVisitaActual + " option:selected").remove();
        }
        else {
    ...
    function QuitarFirma() {
        ...
        let lsUsuariosFirmaVisitaActual = ".lstUsuariosFirma_" + $("#<%=hdTIDFolio.ClientID%>").val();
        let lstFirmasVisitaActual = ".lstFirmas_" + $("#<%=hdTIDFolio.ClientID%>").val();
        if ($(lstFirmasVisitaActual + " option:selected").length > 0) {
            $(lsUsuariosFirmaVisitaActual).append($("<option></option>").attr("value", $(lstFirmasVisitaActual + " option:selected").val()).text($(lstFirmasVisitaActual + " option[value='" + $(lstFirmasVisitaActual).val() + "']").text()));
            $(lstFirmasVisitaActual + " option:selected").remove();
        }
        else {
            alert("Selecciona una firma")
        }
    ...
    function AgregarRubrica() {
        ...
        let lstUsuariosRubricaVisitaActual = ".lstUsuariosRubrica_" + $("#<%=hdTIDFolio.ClientID%>").val();
        let lstRubricasVisitaActual = ".lstRubricas_" + $("#<%=hdTIDFolio.ClientID%>").val();
        if ($(lstUsuariosRubricaVisitaActual + " option:selected").length > 0) {
            $(lstRubricasVisitaActual).append($("<option></option>").attr("value", $(lstUsuariosRubricaVisitaActual).val()).text($(lstUsuariosRubricaVisitaActual + " option[value='" + $(lstUsuariosRubricaVisitaActual).val() + "']").text()));
            $(lstRubricasVisitaActual).val($(lstUsuariosRubricaVisitaActual).val());
            $(lstUsuariosRubricaVisitaActual + " option:selected").remove();
        }
        else {
    ...
    function QuitarRubrica() {
        ...
        let lstUsuariosRubricaVisitaActual = ".lstUsuariosRubrica_" + $("#<%=hdTIDFolio.ClientID%>").val();
        let lstRubricasVisitaActual = ".lstRubricas_" + $("#<%=hdTIDFolio.ClientID%>").val();
        if ($(lstRubricasVisitaActual + " option:selected").length > 0) {
            $(lstUsuariosRubricaVisitaActual).append($("<option></option>").attr("value", $(lstRubricasVisitaActual).val()).text($(lstRubricasVisitaActual + " option[value='" + $(lstRubricasVisitaActual).val() + "']").text()));
            $(lstRubricasVisitaActual + " option:selected").remove();
        }
        else {
            alert("Selecciona una rubrica")
        }
    ...
    function ObtenerFirmas(Origen) {
        ...
        if (Origen == 1) {
            ...
            param_data = '{Area: ' + valueddlAreaFirmaSelected + ', EsSIE: ' + EsSIE + ' }'
        }
        else {
            let valueddlAreaRubricaSelected = $(".ddlAreaRubrica_" + $("#<%=hdTIDFolio.ClientID%>").val()).val();
            param_data = '{Area: ' + valueddlAreaRubricaSelected + ', EsSIE: ' + EsSIE + ' }'
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                let listUsuariosFirmasVisita = ".lstUsuariosFirma_" + $("#<%=hdTIDFolio.ClientID%>").val();
                let listFirmasVisitaActual = ".lstFirmas_" + $("#<%=hdTIDFolio.ClientID%>").val();
                if (Origen == 1) {
                    $(listUsuariosFirmasVisita).html("")
                    $(listFirmasVisitaActual).html("")
                    $.each(data.d, function () {
                        //$("#<%=lstUsuariosFirma.ClientID%>").append($("<option></option>").attr("value", this['Value']).text(this['Text']));
                        $(listUsuariosFirmasVisita).append($("<option></option>").attr("value", this['Value']).text(this['Text']));
                    });
                }
                else {
                    let listUsuariosRubricaVisitaActual = ".lstUsuariosRubrica_" + $("#<%=hdTIDFolio.ClientID%>").val();
                    let listRubricasVisitaActual = ".lstRubricas_" + $("#<%=hdTIDFolio.ClientID%>").val();
                    $(listUsuariosRubricaVisitaActual).html("")
                    $(listRubricasVisitaActual).html("")
                    $.each(data.d, function () {
                        $(listUsuariosRubricaVisitaActual).append($("<option></option>").attr("value", this['Value']).text(this['Text']));
                    });
                }
            },
            error: function (result) {
...
<asp:UpdatePanel ID="upGridDocs" runat="server" UpdateMode="Always">
    <ContentTemplate>
        ...
        <div id="divRegistroSICOD" style="display: none" runat="server">
            <table width="100%">
                <tr>
